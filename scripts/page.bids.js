...
document.getElementById("btn-new-bid").onclick = async () => {
    ...
    // Send bid to API
    const bidResponse = await fetch(`https://api.noroff.dev/api/v1/auction/listings/${listing.id}/bids`, {
        method: "POST",
    ...
    if (!bidResponse.ok) {
        showNetworkError(await bidResponse.json());
        return;
    }

    // Update user credits and session storage
    user.credits -= amount;
    sessionStorage.setItem("user", JSON.stringify(user));
    document.getElementById("user_credits").innerText = user.credits + "💳";

    showMessageBox("Success", "Bid placed successfully!");
    location.reload();
};
